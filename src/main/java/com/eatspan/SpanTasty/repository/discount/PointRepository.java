...
public interface PointRepository extends JpaRepository<Point, Integer> {
	...
	Page<Point> findByMemberIdOrderByPointIdDesc(Integer memberId,Pageable pageable);
	
	@Query("select p from Point p where p.pointChange > 0 AND p.memberId = :memberId Order by p.pointId DESC")
	Page<Point> findByMemberIdOrderByPointIdDescGet(@Param(value = "memberId") Integer memberId,Pageable pageable);
	...
	@Query(nativeQuery = true, value = """
	        ...
	        SELECT 
					COALESCE(ps.member_id, m.member_id) AS memberId, 
	       ...
	       FULL OUTER JOIN members m ON ps.member_id = m.member_id
